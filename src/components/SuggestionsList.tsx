...
import { scanNow, getStatus } from '../services/gmailScan';
...
/**
 * Email-suggestion review queue. Self-contained (scan controls + editable list)
 * so it can render both on the standalone /suggestions route and inline in Inbox.
 * The `title` controls the section heading level for each host.
 */
export default function SuggestionsList({ title = 'Email suggestions' }: { title?: string }) {
  ...
  // Load the last-scan time so the user knows how fresh these suggestions are.
  useEffect(() => {
    getStatus()
      .then((s) => setLastScanAt(s.lastScanAt))
      .catch(() => {});
  }, []);
  ...
  return (
    <>
      <div className="suggestions-header">
        <h2 className="inbox-section-title">{title}</h2>
        <div className="suggestions-scan">
          ...
          {lastScanAt && (
            <span
              className="suggestions-last-scan"
              title={format(new Date(lastScanAt), "MMM d, yyyy 'at' h:mm a")}
            >
